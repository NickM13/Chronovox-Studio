...
Container* ModelOverlay::init(Editor* p_editor) {
	...
	menuBar->addElement("", submenu);
	menuBar->addElement("View", new CMenubar::MenuButton("Focus to Selected", GKey::KeyBind(GLFW_KEY_SPACE, 0), []() { m_editor->getModel()->focus(); }));
	...
	m_container->addComponent(new ContainerPanel("GUI_TOOLBAR", "", { 0, 68 }, { 32, -24 },
		(Sint8)Component::BorderFlag::RIGHT), Component::Anchor::NONE, Component::Anchor::BOTTOM_LEFT)
	...
	m_container->findComponent("GUI_DETAILS")->addComponent(new Container("GUI_COLOR", { 0, 0 }, { 256, 200 }, true))->setPriorityLayer(6);
	...
	m_container->findComponent("GUI_DETAILS\\GUI_COLOR")->addComponent(new CSliderV("SS", "", Vector2<Sint32>(68, 20), 160, 100))
		->setPressFunction([]() {
		m_colorOverlay->setSat(m_container->findComponent("GUI_DETAILS\\GUI_COLOR\\S")->getValue());
		m_container->findComponent("GUI_DETAILS\\GUI_COLOR\\S")->setValue(m_container->findComponent("GUI_DETAILS\\GUI_COLOR\\SS")->getValue());
	});

	m_container->findComponent("GUI_DETAILS")->addComponent(new Container("GUI_MATRICES", { 0, 200 }, { 0, 0 }, true),
