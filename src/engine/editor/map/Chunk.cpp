#include "engine\editor\map\Chunk.h"
...
Chunk::Chunk(Sint16 p_id, std::string p_name, std::string p_parent, glm::vec3 p_pos, glm::ivec3 p_size) {
	m_id = p_id;
	m_name = p_name;
	m_parent = p_parent;
	m_size = p_size;
...
void Chunk::setSize(glm::ivec3 p_size) {
...
void Chunk::setPosition(glm::vec3 p_pos) {
	m_pos = p_pos;
	m_pos = glm::round(m_pos * glm::vec3(10)) / glm::vec3(10);
}
void Chunk::addPosition(glm::vec3 p_pos) {
	m_pos = m_pos + p_pos;
	m_pos = glm::round(m_pos * glm::vec3(10)) / glm::vec3(10);
}

bool Chunk::setVoxel(glm::ivec3 p_pos, Voxel p_voxel) {
...
Voxel Chunk::getVoxel(glm::ivec3 p_pos) {
	return DVoxel::getVoxel(m_voxelData[p_pos.x + 1][p_pos.y + 1][p_pos.z + 1]);
}
Uint16*** Chunk::getVoxelData() {
	return m_voxelData;
}
Uint16 Chunk::getVoxelId(glm::ivec3 p_pos) {
	return m_voxelData[p_pos.x + 1][p_pos.y + 1][p_pos.z + 1];
}
void Chunk::shiftVoxels(glm::ivec3 d) {
...
void Chunk::flip(Sint8 p_axes, glm::vec3 p_focus) {
...
void Chunk::rotate(Sint8 p_axes, glm::vec3 p_focus) {
	glm::ivec3 _size;
	Voxel ***_data = new Voxel**[m_size.x];
	glm::vec3 center = p_focus - getCenter();
	for(Uint16 x = 0; x < m_size.x; x++) {
...
void Chunk::scale(glm::vec3 p_scalar, glm::vec3 p_focus) {
...
void Chunk::autoResize(glm::ivec3* inner, glm::ivec3* ext) {
...
bool Chunk::containsPoint(glm::ivec3 p_point) {
...
void Chunk::renderChunkWireframe() {
	Shader::pushMatrixModel();
	Shader::transformModel(glm::translate(glm::mat4(), glm::vec3(m_pos.x, m_pos.y, m_pos.z)));
...
void Chunk::renderOutline(OutlineType p_outline) {
