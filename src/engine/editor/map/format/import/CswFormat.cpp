#include "engine\editor\map\format\import\CswFormat.h"
#include "engine\editor\map\format\GFormat.h"
...
Uint32 CswFormat::m_index = 0;
char* CswFormat::m_data = 0;

bool CswFormat::save(std::string p_fileName, std::vector<Chunk*>* p_matrixList) {
	std::ofstream _file;
...
bool CswFormat::load(std::string p_fileName, std::vector<Chunk*>* p_matrixList) {
	std::ifstream _file;
	Uint32 _length;
	bool _success = true;
...
bool CswFormat::load1(std::vector<Chunk*>* p_matrixList) {
	Sint32 csm, version, blank, matrixCount;
	csm = FileExt::readInt(m_data, m_index);
	version = FileExt::readInt(m_data, m_index);
	blank = FileExt::readInt(m_data, m_index);
	...
	for (Uint16 i = 0; i < matrixCount; i++) {
		...
		m = new Chunk(i, name, "", glm::vec3(pos) / glm::vec3(2), size);
...
bool CswFormat::load2(std::vector<Chunk*>* p_matrixList) {
	Sint32 csm, version, blank, matrixCount;
	csm = FileExt::readInt(m_data, m_index);
	version = FileExt::readInt(m_data, m_index);
	blank = FileExt::readInt(m_data, m_index);
	...
	for (Uint16 i = 0; i < matrixCount; i++) {
		...
		matrix = new Chunk(i, name, parent, glm::vec3(pos) / glm::vec3(2), size);
...
bool CswFormat::load3(std::vector<Chunk*>* p_matrixList) {
	Sint32 csm, version, blank, matrixCount;
	csm = FileExt::readInt(m_data, m_index);
	version = FileExt::readInt(m_data, m_index);
	blank = FileExt::readInt(m_data, m_index);
	...
	for (Uint16 i = 0; i < matrixCount; i++) {
		...
		matrix = new Chunk(i, name, parent, glm::vec3(pos) / glm::vec3(10), size);
...
bool CswFormat::load4(std::vector<Chunk*>* p_matrixList) {
	Sint32 csm, version, blank, matrixCount;
	csm = FileExt::readInt(m_data, m_index);
	version = FileExt::readInt(m_data, m_index);
	blank = FileExt::readInt(m_data, m_index);
