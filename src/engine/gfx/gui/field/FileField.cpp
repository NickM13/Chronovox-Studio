...
FileField::FileField(std::string p_compName, std::string p_title, Vector2<Sint32> p_pos, Vector2<Sint32> p_size, Theme p_colorTheme, bool p_limitField)
	...
	glPushMatrix();
	{
		...
		if(m_title != "" || m_text[0] != "" || m_text.size() > 1) {
			for(Uint16 i = 0; i < m_text.size(); i++) {
				...
				Font::print(m_text[i], 2 - std::fmaxf(0, Font::getMessageWidth(m_text[i]).x - m_size.x + 4), Sint32((i + 0.5f) * Font::getSpacingHeight()));
				MScissor::getInstance().pop();
			}
			if(m_selected != 0 && (fmod(glfwGetTime(), 0.5) < 0.25))
				Font::print("|", Sint32(Font::getMessageWidth(m_text[m_cursorPos.y].substr(0, m_cursorPos.x)).x + 1) - std::fmaxf(0, Font::getMessageWidth(m_text[m_cursorPos.y]).x - m_size.x + 4), Sint32((m_cursorPos.y + 0.5f) * Font::getSpacingHeight()));
		}
		else {
			if(m_selected != 0) {
				m_colorTheme.m_text.useColor();
