	...
	glPushMatrix();
	{
		...
		if(m_title != "" || m_text[0] != "" || m_text.size() > 1)
		{
			...
			if(m_selected != 0 && (fmod(glfwGetTime(), 0.5) < 0.25))
				Font::getInstance().print("|", Sint32(Font::getInstance().getMessageWidth(m_text[m_cursorPos.y].substr(0, m_cursorPos.x)).x + 1) - max(0, Font::getInstance().getMessageWidth(m_text[m_cursorPos.y]).x - m_size.x + 4), Sint32((m_cursorPos.y + 0.5f) * Font::getInstance().getSpacingHeight()));
		}
		else
		{
			if(m_selected != 0)
