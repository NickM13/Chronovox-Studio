...
CTabBar::CTabBar(std::string p_compName, Vector2<Sint32> p_pos, Vector2<Sint32> p_size, Theme p_colorTheme)
	...
	m_selected = 0;
	m_updated = 0;
	m_buttonLShift = new CButton("", "<", { 0, 0 }, { 20, m_size.y }, CButton::RenderStyle::ALL);
	m_buttonRShift = new CButton("", ">", { m_size.x - 20, 0 }, { 20, m_size.y }, CButton::RenderStyle::ALL);
	m_maxScroll = 0;
}
...
void CTabBar::render() {
	...
	for (Sint32 i = 0; i < Sint32(m_tabList.size()); i++)
	{
		...
		_button->render();
		Shader::translate(glm::vec3((GLfloat)(_button->getSize().x + 4), 0.f, 0.f));
	}
	_button->setTitle("+");
	_button->setSize(Vector2<Sint32>(20, m_size.y));
